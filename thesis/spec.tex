...
The formalization of the usage analysis operates on a simply untyped lambda calculus, defined in \cref{fig:exp}. 
...
The answer to the first question above then consists of identification of \emph{one-shot} lambdas, \eg the cases where the body of the lambda expression is called at most once, relative to its defining scope.
...
\Cref{fig:usg} depicts the $\sUse$ and the $\sUsage$ lattice, giving the language we used in \cref{sec:dmd} proper meaning. 
...
\item The most conservative analysis result would be the top of the lattice $U$, representing an unknown use beyond reduction to WHNF.
      ...
      An unknown use on a function expression can be interpreted as a use of $C^\omega(U)$ by the last equality: 
...
Thus, every identifier has an associated $\sUsage$, which describes how often and how, if at all, the identifier was used.
...
\begin{itemize}
\item The pattern match variable \hsinl{y} has usage $1*U$, \hsinl{x} is absent ($A$).
\item The usage on pattern match variables translate to a pair use on \hsinl{(a,b)} of $U(A,1*U)$.
...
The following expressions hint at the fact that, in general, right-hand sides of bindings can be put under call uses $C^{n_1}(C^{n_2}(\ldots C^{n_k}(U)\ldots))$ for any $n_i\in\sMulti$.
...
A use of $U$ on the expression manifests in two sequential usages $1*C^1(C^1(U))$ of \hsinl{f}, each corresponding to a single call with two arguments. 
These combine to an identifier usage of $\omega*C^\omega(C^1(U))$: 
After all, the lambda expression, which the function bound to \hsinl{f} reduces to, is called twice, resulting in two (non-shared!) values. 
...
This is the same argument that \textcite[Appendix~C.2]{warnsbrough} makes with his language of \emph{use} \vs \emph{demand}.
...
If this is put under the typical use $U$, closer look reveals that \hsinl{k} is used as a continuation inside \hsinl{f}, \eg has the one-shot usage $1*C^1(U)$.
...
The usage signatures $\top$ and $\bot$ hint at the fact that there is some lattice structure, so here comes the definition for $\llub$:
\begin{alignat*}{1}
...
For another noteworthy example, the \hsinl{seq} operator would have a usage signature of $1*HU \to 1*U \to \top$.
...
Knowing that \hsinl{x} is not forced together with \hsinl{y}\footnote{Prior to looking at the right-hand side of \hsinl{y} is, of course.}, we avoid a self-edge on \hsinl{x} after analysing the right-hand side of \hsinl{y}.

Consequently, \hsinl{x} is recognized of only being evaluated once. 
Other usage analyses like that of GHC's Cardinality Analysis \parencite{card} only model self-loops in the underlying co-call structure (\eg usage multiplicity), so they would fail to recognize the absent edge between \hsinl{x} and \hsinl{y} and conservatively assume \hsinl{x} to be called more than once\footnote{It is worth pointing out that this is only the case when \hsinl{y} is a thunk (\eg not in WHNF), as this imprecision pertains to the \letupsc rule of \textcite{card}. 
...
\Cref{fig:lub} lists all definitions of $\llub$ for the various algebras involved.
...
We'd like that $u$ is some kind of call use, but if you give a programmer enough rope to hang himself (\eg \hsinl{unsafeCoerce}) to bypass the type system, it is possible for $u$ to be incomparable to a call use, \ie $U(1*U)$. 
...
\subsection{\keyword{case} Expressions}
...
We primarily need \keyword{if} expressions for the sake of illustrative examples, with non-complete co-call graphs introduced by joining the two branches. 
...
The substitution operator is more precise than plain sequential composition by selecting \emph{only a subset} $N$ of variables from $\dom{\varphi'}$ which are to be sequentially composed with $\dom{\varphi_i}$.
...
To wrap this up, we visit some examples contrasting \letupsc style analysis with \letdnsc, discussing strengths and weaknesses.
...
\begin{example}
  We even found a minimal counter-example where \letupsc{} with co-call graphs yields worse results than an approach based on \letdnsc{}.
  ...
  The expression bound to \hsinl{x} is a thunk, so the binding is unleashed \letupsc-style.

  Knowing that the usage signature of \hsinl{seq} is $1*HU \to 1*U \to \top$, the usage type of the inner-most \hsinl{if} expression is
  \[
    \lTriple{\edge{f}{x}}{\left[f\mapsto C^1(U), x\mapsto U\right]}{\top}
  \]

  Notice how there is a co-call edge from \hsinl{f} to \hsinl{x} due to the \hsinl{then} branch, but also that \hsinl{f} exposed to usage $1*C^1(U)$ because of the \hsinl{else} branch.
  Substituting the usage type of its bound expression for \hsinl{x} will then expose \hsinl{f} to usage $\omega*C^\omega(U) = \omega*U$ because of the existing co-call edge.

  That's clearly imprecise: \hsinl{f} is only ever called once, even if in the \hsinl{then} branch it is brought to WHNF (\eg put under head use $HU$) and then called by evaluation of \hsinl{x}, resulting in a usage of $\omega*C^1(U)$.
  
  Indeed, that is what we would find out if we unleashed \hsinl{x} \letdnsc-style. 
  As should be clear by now, we don't do this because in general there might be multiple calls to \hsinl{x} and the work of bringing down its bound expression to WHNF is shared.
...
In particular, the distinction between functions and thunks and how substitution in co-call graphs works are delicate technicalities.
...
Of course, just showing existence of the fixed-point does not imply termination, let alone any guarantees with respect to runtime complexity. 
\Cref{sec:approx} discusses efficient approximation of usage transformers in more detail.
...
Call Arity annotates every \keyword{let}-binder with this \emph{call arity}, the arity to which the bound expression can be $\eta$-expanded without losing any sharing.
...
A head use can be treated similarly to the absent case, with the exception that for zero arity we should not expand at all, to avoid surprising interplay with \hsinl{seq}.
...
However, it is not immediately obvious how the analysis strategy for \keyword{let} bindings is subsumed.
Because Call Arity does arity expansion based on use sites, it employs a \letupsc strategy for all bindings.
We talked about the trade-offs of \letupsc \vs \letdnsc in \cref{sec:graph} and \cref{sec:let}, which are adequately summarised by saying that our analysis never yields worse results than always analysing \letupsc-style would.

Other than that, we improve on Call Arity in a number of ways:

\begin{description}
  \item[Usage signatures] The mixed \letdnsc/\letupsc style allows us to unleash usage signatures at call sites for higher-order information, where Call Arity effectively assumes a usage signature of $\top$.
...
\begin{description}
  \item[Unified \letupsc/\letdnsc] The strictly separated \letupsc and \letdnsc rules have been superseded by an approach that always unleashes usage signatures at call sites \letdnsc-style (also for thunks). Also, in the \letdnsc case we put the bound expression under the precise incoming use instead of digesting the bound expression for the case of a single call with manifest arity (the number of leading lambdas, that is).
  \item[Co-call graphs] We gain precision for the \letupsc case by tracking the co-call relationship (\cf \cref{sec:graph} and \cref{sec:let}).
  ...
  \item[Ideal specification] The transfer function serving as a specification captures the essence of the analysis without leaking any implementation details, such as how approximate the usage transformers unleashed through \letdnsc are.
\end{description}

We will revisit the issues of how approximate we want usage transformers to be in \cref{sec:approx}.

Specifically, we recover Cardinality Analysis by forgetting information and being more approximate in the \letdnsc case, so that a single analysis pass over the bound expression is enough.
...
\begin{itemize}
  \item Modify the $\letdown{\uscore}{\uscore}$ combinator (\cf \cref{sec:let}) to replace usage signatures by $\bot$ for thunks and to distinguish only the three cases of 
    ...
    \end{enumerate}
  \item 
    Track only self-edges in the co-call graph and conservatively assume all other edges exist. This corresponds to tracking per-identifier usage multiplicity separately instead of in a co-call graph, see \cref{sec:graph}.
\end{itemize}
