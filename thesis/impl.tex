...
\section{Solving the Data-flow Problem}
...
Without having done any benchmarks to justify, we decided to only allocate \hsinl{FrameworkNode}s where we needed to break cycles in the data-flow graph.
...
From there, the iteration algorithm begins to explore the data-flow graph in a depth-first fashion, which -- as we already pointed out at the begin of this section -- is crucial for termination, as the graph is infinite, while the nodes reachable from \hsinl{(root, topUse)} is finite \tod{Maybe some kind of visualisation?}.

\begin{example}
  Consider the following example module, computing the factorial of 12:
  \begin{haskellcode}
  ...
  \begin{haskellcode}
    ...
    in let main = print (fac 12)
       in (main) -- This is a 1-tuple, e.g. a boxed constructor
  \end{haskellcode}
...
\todo{Other stuff: Where and what we annotate, Why not through Map, Coercions, imported Ids}
