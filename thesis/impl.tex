...
A real implementation would thus thread an annotated expression as an additional output. 
Our usage analysis is no different, in fact we annotate the same information as \textcite{card} does:
...
We see in \cref{sec:solve} an example where use on a call site produces too optimistic annotations, which of course are not used.
