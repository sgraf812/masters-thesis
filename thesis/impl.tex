...
This is because the whole scope of non-exported top-level bindings is statically known.
...
GHC's rewrite rules and vectorisation declarations possibly mention identifiers which are neither exported, nor otherwise reachable.
These must be included in the root set!
...
We made sure that edge tests were still efficient by storing the represented graph's node-indexed adjacency sets (witnessed by an isomorphism $\sGraph \simeq \sVar \to \mathcal{P}(\sVar)$ modulo symmetry) directly in a \hsinl{newtype} over an \hsinl{IntMap IntSet}, leaning on unique integer keys GHC assigns to variables.
...
Additionally, associated with each node is a \emph{transfer function} which has the single responsibility of recomputing analysis information in terms of the current state of its dependencies.
...
\begin{figure}[h]
  ...
  \begin{haskellcode}
    ...
    dependOn
  \end{haskellcode}
  \caption{The essence of the \hsinl{Worklist} module.}
  \label{fig:worklist}
\end{figure}
...
This is why we decided to only allocate \hsinl{FrameworkNode}s where we had to break cycles in the data-flow graph.
...
From there, the iteration algorithm begins to explore the data-flow graph in a depth-first fashion. 
As we already pointed out at the begin of this section, this is crucial for termination: 
The graph itself is infinite, while the set of nodes reachable from $(\hsinl{root}, U)$ is finite.
...
\begin{example}
  ...
  %\input{fig/titlefig.tex}

  \newcommand{\fw}[1]{\overrightarrow{#1}}
  \newcommand{\bw}[1]{\overleftarrow{#1}}
  \begin{figure}
    \centering
    \begin{tikzpicture}[shorten >=1pt,x=10em,y=7em,auto]
      ...
      \path[->,line width=1pt] 
        (root) edge node {$\fw{1},\bw{12},\bw{20}$} (let1)
        (let1) edge [loop above] node {$\fw{2},\bw{2},\bw{19}$} ()
               edge node {$\fw{3},\bw{9}$} (let2)
               edge node {$\fw{9},\bw{11},\bw{18}$} (facn)
        (let2) edge node {$\fw{4},\bw{8}$} (main)
        (main) edge node {$\fw{5},\bw{7},\bw{17}$} (fac1)
        (facn) edge node {$\fw{10},\bw{10},\bw{16}$} (fac1)
        (fac1) edge [loop below] node {$\fw{6},\bw{6},\bw{14},\bw{15}$} ();
    \end{tikzpicture}
    \caption{
       Relevant part of the data-flow framework for the \hsinl{fac} example. 
       The module node is drawn in red, \letupsc nodes are blue and \letdnsc nodes are green.
  ...
  \begin{enumerate}
    ...
    \item
      ...
      Nothing interesting happens to the literal $12$, thus the usage type of the \letdnsc node for $(\hsinl{main}, U)$ is $\lTriple{\{(\hsinl{fac}, \hsinl{fac})\}}{\maplit{\hsinl{fac}}{U}}{\top}$.
    ...
    \item
      ...
      Within the body, \hsinl{main} is exposed to usage $\omega*U$.
    ...
    \item
      ...
      More important, the lambda is not one-shot and the argument binder for \hsinl{n} is not single-entry, contrary to the optimistic situation under use $C^1(U)$.
...
This work, in particular the \hsinl{Worklist} module, solidifies these findings and provides a rather elegant embedded domain-specific language for constructing such frameworks.
Eventually, we plan to release this module as a separate package on Hackage, after it has gone through some polishing.

We could see this kind of graph-based data-flow iteration becoming more feasible by associating a \hsinl{FrameworkNode} with each \hsinl{CoreExpr} constructor (or rather each subexpression).
This would get rid of the extra book-keeping required to assign good priorities to nodes through the \hsinl{FrameworkBuilder} code, which is actually inessential to the analysis and mostly contained in \hsinl{buildAnalFramework} in \hsinl{UsageAnal.Analysis}.
...
Of course, \hsinl{uncurry}ing didn't change the actual semantics, but modeling each point separately means that prior to reaching the fixed-point, there are unstable intermediate approximations which might not be monotone.
It turns out that convergence depends even on these unstable approximations to be monotone.

This is easy to reproduce by removing all mentions of the \hsinl{monotonize} function from the main analysis module and then running the test \texttt{joao-circular} with a \texttt{devel2} flavored build of the compiler.
The file \texttt{Visfun\_Lazy.hs} will violate a monotonicity check while updating the value of a framework node.
Unfortunately, that file is rather huge and convoluted, so a detailed analysis is out of question.
