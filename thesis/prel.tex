...
That is not the case, because the reduction to WHNF of \hsinl{f}'s bound expression will be shared, thus \hsinl{x} is actually single-entry.
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}[shorten >=1pt,x=10em,y=5em,auto]
    ...
    \tikzstyle{n}=[draw,shape=ellipse]
    \node (usage) at (0,1) [n] {Usage};
    \node (strict) at (1,1) [n] {Strictness};
    \node (cpr) at (2,1) [n] {CPR};
    \node (ww) at (1,0) [n] {Worker/Wrapper};
    \node (simp) at (1,2) [n] {Simplifier};
    \node (prep) at (0,2) [n] {CorePrep};
