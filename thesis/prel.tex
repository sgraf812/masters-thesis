...
There is some pointless packing and unpacking involved in calling \hsinl{f} with the pair literal:
After all, \hsinl{f} doesn't use the first component of the argument at all!

If \hsinl{f} can be inlined, this packing/unpacking immediately gets optimised away by the simplifier, as \hsinl{a} is not mentioned anywhere in the body of \hsinl{f}.
...
Since \hsinl{f} is now a thunk (\eg binds an expression that is not in WHNF), expanding \hsinl{f} to \emph{call arity} (the minimum arity of each call), which is two, duplicates the work associated with evaluating \hsinl{expensive}, which would otherwise be shared between the two calls.
...
Thus, Call Arity tracks in its abstract domain for each binder whether it was just called once and the call arity (as in minimum arity of each call).
